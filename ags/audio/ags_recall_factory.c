  ...
  if((AGS_RECALL_FACTORY_PLAY & (create_flags)) != 0){
	...
	while(current != NULL){
	  ...
	  while((list = ags_recall_template_find_type(list,
	    ...
	    if(AGS_RECALL_CHANNEL(list->data)->destination == current){
	      found_prepare = TRUE;
	      break;
	    }
				  ...
				  guint create_flags, guint recall_flags)
{
  ...
  if((AGS_RECALL_FACTORY_PLAY & (create_flags)) != 0){
	...
	ags_recall_set_flags(AGS_RECALL(analyse_channel), (AGS_RECALL_TEMPLATE |
							   (((AGS_RECALL_FACTORY_OUTPUT & create_flags) != 0) ? AGS_RECALL_OUTPUT_ORIENTATED: AGS_RECALL_INPUT_ORIENTATED) |
							   AGS_RECALL_PLAYBACK |
							   AGS_RECALL_SEQUENCER |
							   AGS_RECALL_NOTATION));
	...
	analyse_channel_run = (AgsAnalyseChannelRun *) g_object_new(AGS_TYPE_ANALYSE_CHANNEL_RUN,
								    ...
								    "recall_container", play_container,
								    NULL);
	ags_recall_set_flags(AGS_RECALL(analyse_channel_run), (AGS_RECALL_TEMPLATE |
							       (((AGS_RECALL_FACTORY_OUTPUT & create_flags) != 0) ? AGS_RECALL_OUTPUT_ORIENTATED: AGS_RECALL_INPUT_ORIENTATED) |
							       AGS_RECALL_PLAYBACK |
							       AGS_RECALL_SEQUENCER |
							       AGS_RECALL_NOTATION));
