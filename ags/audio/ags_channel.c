...
void ags_channel_set_recycling_emit_changed(AgsChannel *channel, AgsChannel *input,
					    ...
					    AgsRecycling *old_first_recycling, AgsRecycling *old_last_recycling,
					    AgsRecycling *changed_old_first_recycling, AgsRecycling *changed_old_last_recycling);
void ags_channel_reset_recycling_context(AgsChannel *channel,
					 AgsRecyclingContext *recycling_context, AgsRecyclingContext *old_recycling_context,
					 GList *recall_id);
void ags_channel_recursive_reset_recycling_context_up(AgsChannel *start, AgsChannel *channel,
						      AgsRecyclingContext *recycling_context, AgsRecyclingContext *old_recycling_context);
void ags_channel_recursive_reset_recycling_context_down_input(AgsChannel *start, AgsChannel *channel,
							      AgsRecyclingContext *recycling_context, AgsRecyclingContext *old_recycling_context);
void ags_channel_recursive_reset_recycling_context_down_output(AgsChannel *start, AgsChannel *channel,
							       AgsRecyclingContext *recycling_context, AgsRecyclingContext *old_recycling_context);
void ags_channel_recursive_set_property_set_property(AgsChannel *channel,
...
void ags_channel_recursive_duplicate_recall_down_input(AgsChannel *output,
						       GList *recall_id_list);
void ags_channel_recursive_duplicate_recall_down(AgsChannel *channel,
						 GList *recall_id_list);
...
void ags_channel_recursive_resolve_recall_down_input(AgsChannel *output,
						     GList *recall_id);
void ags_channel_recursive_resolve_recall_down(AgsChannel *channel,
					       GList *recall_id);
...
void ags_channel_recursive_init_recall_down_input(AgsChannel *output,
						  GList *recall_id,
						  gint stage);
void ags_channel_recursive_init_recall_down(AgsChannel *channel,
					    GList *recall_id,
					    gint stage);
			  ...
			  gboolean update, gboolean destroy_old)
{
  ...
  while(recall_id != NULL){
    ...
    current_recall_id = g_object_new(AGS_TYPE_RECALL_ID,
				     ...
				     "recycling-context", AGS_RECALL_ID(recall_id->data)->recycling_context,
				     NULL);
...
void
ags_channel_reset_recycling_context(AgsChannel *channel,
				    AgsRecyclingContext *recycling_context, AgsRecyclingContext *old_recycling_context,
				    GList *recall_id)
{
  while(recall_id != NULL){
...
void
ags_channel_recursive_reset_recycling_context_up(AgsChannel *start, AgsChannel *channel,
						 AgsRecyclingContext *recycling_context, AgsRecyclingContext *old_recycling_context)
{
  AgsAudio *audio;
  ...
  if(is_output){
    ...
    ags_channel_reset_recycling_context(current,
					recycling_context, old_recycling_context,
					audio->recall_id);
  ...
  if(AGS_IS_INPUT(current) &&
    ...
    while(input != NULL){
      if(input != current){
	link = input->link;
	ags_channel_reset_recycling_context(input,
					    recycling_context, old_recycling_context,
					    input->recall_id);

	/* traverse the tree */
	ags_channel_recursive_reset_recycling_context_down_output(start, link,
								  recycling_context, old_recycling_context);
      }
...
void
ags_channel_recursive_reset_recycling_context_down_input(AgsChannel *start, AgsChannel *output,
							 AgsRecyclingContext *recycling_context, AgsRecyclingContext *old_recycling_context)
{
  AgsAudio *audio;
  AgsChannel *input;
...
void
ags_channel_recursive_reset_recycling_context_down_output(AgsChannel *start, AgsChannel *output,
							  AgsRecyclingContext *recycling_context, AgsRecyclingContext *old_recycling_context)
{
  AgsAudio *audio;
  ...
  ags_channel_reset_recycling_context(output,
				      recycling_context, old_recycling_context,
				      output->recall_id);
  ...
  /* traverse the tree */
  ags_channel_recursive_reset_recycling_context_down_input(start, output,
							   recycling_context, old_recycling_context);
}
					      ...
					      AgsRecyclingContext *recycling_context)
{
  ...
  if(!is_output){
    ags_channel_recursive_reset_recycling_context_down_output(channel, channel->link,
							      recycling_context, old_recycling_context);
    ags_channel_recursive_reset_recycling_context_up(channel, channel,
						     recycling_context, old_recycling_context);
  }else{
    ags_channel_recursive_reset_recycling_context_down_output(channel, channel,
							      recycling_context, old_recycling_context);
    ags_channel_recursive_reset_recycling_context_up(channel, channel->link,
						     recycling_context, old_recycling_context);
  }
}
...
void
ags_channel_recursive_set_property_set_property(AgsChannel *channel,
						GParameter *parameter, gint n_params)
{
  guint i;
					...
					GParameter *parameter, gint n_params)
{
  ...
  ags_channel_recursive_set_property_set_property(channel,
						  parameter, n_params);
					      ...
					      GParameter *parameter, gint n_params)
{
  ...
  while(input != NULL){
    ags_channel_recursive_set_property_set_property(input,
						    parameter, n_params);
 ...
 */
AgsRecallID*
ags_channel_recursive_play_init_arrange_recall_id_up(AgsChannel *channel,
						     AgsRecallID *recall_id,
						     gboolean initially_unowned,
						     gboolean do_playback, gboolean do_sequencer, gboolean do_notation)
{
...
AgsRecallID**
ags_channel_recursive_play_init_arrange_recall_id_down_input(AgsChannel *output,
							     AgsRecallID *default_recall_id,
							     gboolean initially_unowned,
							     gboolean do_playback, gboolean do_sequencer, gboolean do_notation)
{
  ...
  if((AGS_AUDIO_ASYNC & (audio->flags)) != 0){
	...
	ags_channel_recursive_play_init_arrange_recall_id_down(input->link,
							       input_recall_id,
							       FALSE,
							       do_playback, do_sequencer, do_notation);
      }
...
AgsRecallID*
ags_channel_recursive_play_init_arrange_recall_id_down(AgsChannel *output,
						       AgsRecallID *recall_id,
						       gboolean initially_unowned,
						       gboolean do_playback, gboolean do_sequencer, gboolean do_notation)
{
 ...
 */
void
ags_channel_recursive_play_init_duplicate_up(AgsChannel *channel,
					     AgsRecallID *recall_id)
{
  AgsAudio *audio;
  AgsChannel *current;
...
void
ags_channel_recursive_play_init_duplicate_down_input(AgsChannel *output,
						     AgsRecallID *default_recall_id)
{
  AgsAudio *audio;
...
void
ags_channel_recursive_play_init_duplicate_down(AgsChannel *output,
					       AgsRecallID *recall_id)
{
  AgsAudio *audio;
 ...
 */
void
ags_channel_recursive_play_init_resolve_up(AgsChannel *channel,
					   AgsRecallID *recall_id)
{
  AgsAudio *audio;
  AgsChannel *current;
...
void
ags_channel_recursive_play_init_resolve_down_input(AgsChannel *output,
						   AgsRecallID *default_recall_id)
{
  AgsAudio *audio;
  AgsChannel *input, *input_start;
...
void
ags_channel_recursive_play_init_resolve_down(AgsChannel *output,
					     AgsRecallID *recall_id)
{
  AgsAudio *audio;
  AgsChannel *input;
 ...
 */
void
ags_channel_recursive_play_init_up(AgsChannel *channel,
				   AgsRecallID *recall_id,
				   gint stage)
{
  AgsAudio *audio;
...
void
ags_channel_recursive_play_init_down_input(AgsChannel *output,
					   AgsRecallID *default_recall_id,
					   gint stage)
{
  AgsAudio *audio;
...
void
ags_channel_recursive_play_init_down(AgsChannel *output,
				     AgsRecallID *recall_id,
				     gint stage)
{
  AgsAudio *audio;
...
void
ags_channel_recursive_play_threaded_up(AgsChannel *channel,
				       AgsRecallID *recall_id,
				       AgsRecyclingThread *recycling_thread,
				       gint stage)
{
...
void
ags_channel_recursive_play_threaded_down_input(AgsChannel *output,
					       AgsRecallID *default_recall_id,
					       AgsRecyclingThread *recycling_thread,
					       gint stage)
{
...
void
ags_channel_recursive_play_threaded_down(AgsChannel *output,
					 AgsRecallID *recall_id,
					 AgsRecyclingThread *recycling_thread,
					 gint stage)
{
...
void
ags_channel_recursive_play_up(AgsChannel *channel,
			      AgsRecallID *recall_id,
			      gint stage)
{
  AgsAudio *audio;
...
void
ags_channel_recursive_play_down_input(AgsChannel *output,
				      AgsRecallID *default_recall_id,
				      gint stage)
{
  AgsAudio *audio;
...
void
ags_channel_recursive_play_down(AgsChannel *output,
				AgsRecallID *recall_id,
				gint stage)
{
  AgsAudio *audio;
...
void
ags_channel_tillrecycling_cancel_up(AgsChannel *channel,
				    AgsRecallID *recall_id)
{
  AgsAudio *audio;
  AgsChannel *current;
...
void
ags_channel_tillrecycling_cancel_down_input(AgsChannel *output,
					    AgsRecallID *default_recall_id)
{
  AgsAudio *audio;
  AgsChannel *input, *input_start;
...
void
ags_channel_tillrecycling_cancel_down(AgsChannel *output,
				      AgsRecallID *recall_id)
{
  AgsAudio *audio;
  AgsRecallID *audio_recall_id, *default_recall_id;
...
/* implementation */
void
ags_audio_reset_recall_id(AgsAudio *audio,
			  GList *recall_id_list, GList *devout_play_list,
			  GList *invalid_recall_id_list)
{
  AgsRecall *recall;
  ...
  while(recall_id_list != NULL){      
	...
	recall_id = g_object_new(AGS_TYPE_RECALL_ID,
				 "recycling", AGS_RECALL_ID(recall_id_list->data)->recycling,
				 "recycling-context", AGS_RECALL_ID(recall_id_list->data)->recycling_context,
				 NULL);
	ags_audio_add_recall_id(audio,
				(GObject *) recall_id);
...
void
ags_channel_reset_recall_id(AgsChannel *channel,
			    GList *recall_id_list, GList *devout_play_list,
			    GList *invalid_recall_id_list)
{
  AgsAudio *audio;
...
void
ags_channel_recursive_reset_audio_duplicate_recall(AgsAudio *audio,
						   GList *recall_id_list)
{
  AgsRecallID *recall_id;
...
void
ags_channel_recursive_reset_channel_duplicate_recall(AgsChannel *channel,
						     GList *recall_id_list)
{
  AgsRecallID *recall_id;
...
void
ags_channel_recursive_reset_audio_resolve_recall(AgsAudio *audio,
						 GList *recall_id_list)
{
  AgsRecallID *recall_id;
...
void
ags_channel_recursive_reset_channel_resolve_recall(AgsChannel *channel,
						   GList *recall_id_list)
{
  AgsRecallID *recall_id;
...
void
ags_channel_recursive_reset_audio_init_recall(AgsAudio *audio,
					      GList *recall_id_list,
					      gint stage)
{
  AgsRecallID *recall_id;
...
void
ags_channel_recursive_reset_channel_init_recall(AgsChannel *channel,
						GList *recall_id_list,
						gint stage)
{
  AgsAudio *audio;
...
GList*
ags_channel_tillrecycling_collect_devout_play_down_input(AgsChannel *output,
							 GList *list, gboolean collect_recall_id)
{
  AgsAudio *audio;
  AgsChannel *current;
  ...
  if((AGS_AUDIO_ASYNC & (audio->flags)) != 0){
	...
	if((AGS_PLAYBACK_PLAYBACK & (g_atomic_int_get(&(AGS_PLAYBACK(current->playback)->flags)))) != 0 &&
	  ...
	  if(collect_recall_id){
	    list = g_list_prepend(list,
				  AGS_PLAYBACK(current->playback)->recall_id[0]);
	  }else{
	    list = g_list_prepend(list,
				  current->playback);
	  }
	...
	if((AGS_PLAYBACK_SEQUENCER & (g_atomic_int_get(&(AGS_PLAYBACK(current->playback)->flags)))) != 0 &&
	  ...
	  if(collect_recall_id){
	    list = g_list_prepend(list,
				  AGS_PLAYBACK(current->playback)->recall_id[1]);
	  }else{
	    list = g_list_prepend(list,
				  current->playback);
	  }
	...
	if((AGS_PLAYBACK_NOTATION & (g_atomic_int_get(&(AGS_PLAYBACK(current->playback)->flags)))) != 0 &&
	  ...
	  if(collect_recall_id){
	    list = g_list_prepend(list,
				  AGS_PLAYBACK(current->playback)->recall_id[2]);
	  }else{
	    list = g_list_prepend(list,
				  current->playback);
	  }
...
GList*
ags_channel_tillrecycling_collect_devout_play_down(AgsChannel *current,
						   GList *list, gboolean collect_recall_id)
{
  AgsAudio *audio;
...
GList*
ags_channel_recursive_collect_devout_play_up(AgsChannel *channel,
					     gboolean collect_recall_id)
{
  AgsAudio *audio;
  AgsChannel *current;
...
GList*
ags_channel_recursive_collect_recycled(AgsChannel *channel,
				       GList *collected_recall_id)
{
  AgsAudio *audio;
  AgsChannel *current;
...
void
ags_channel_recursive_reset_recall_id_down_input(AgsChannel *output,
						 GList *recall_id_list, GList *devout_play_list,
						 GList *invalid_recall_id_list)
{
  AgsAudio *audio;
...
void
ags_channel_recursive_reset_recall_id_down(AgsChannel *current,
					   GList *recall_id_list, GList *devout_play_list,
					   GList *invalid_recall_id_list)
{
  AgsAudio *audio;
  ...
  if((AGS_AUDIO_OUTPUT_HAS_RECYCLING & (audio->flags)) == 0){
    ...
    while(recall_id_list != NULL){
      ...
      recycling_context = (AgsRecyclingContext *) g_object_new(AGS_TYPE_RECYCLING_CONTEXT,
							       "length", (guint64) recycling_length,
							       "parent", AGS_RECALL_ID(recall_id_list->data)->recycling_context,
							       NULL);
      ags_audio_add_recycling_context(audio,
				      (GObject *) recycling_context);
	...
	recall_id->flags |= AGS_RECALL_ID_NOTATION;
      ...
      recycling_context = (AgsRecyclingContext *) g_object_new(AGS_TYPE_RECYCLING_CONTEXT,
							       "length", (guint64) recycling_length,
							       "parent", AGS_RECALL_ID(output_recall_id_list->data)->recycling_context,
							       NULL);
      ags_audio_add_recycling_context(audio,
				      (GObject *) recycling_context);
...
void
ags_channel_tillrecycling_reset_recall_id_up(AgsChannel *channel,
					     GList *recall_id_list, GList *devout_play_list,
					     GList *invalid_recall_id_list)
{
  AgsAudio *audio;
...
void
ags_channel_recursive_unset_recall_id_down_input(AgsChannel *output,
						 GList *invalid_recall_id_list)
{
  AgsAudio *audio;
  AgsChannel *current;
...
void
ags_channel_recursive_unset_recall_id_down(AgsChannel *current,
					   GList *invalid_recall_id_list)
{
  AgsAudio *audio;
  GList *next_invalid_recall_id_list;
...
void
ags_channel_tillrecycling_unset_recall_id_up(AgsChannel *channel,
					     GList *invalid_recall_id_list)
{
  AgsAudio *audio;
  AgsChannel *current;
...
void
ags_channel_recursive_duplicate_recall_down_input(AgsChannel *output,
						  GList *recall_id_list)
{
  AgsAudio *audio;
  AgsChannel *current;
...
void
ags_channel_recursive_duplicate_recall_down(AgsChannel *current,
					    GList *recall_id_list)
{
  AgsAudio *audio;
...
void
ags_channel_tillrecycling_duplicate_recall_up(AgsChannel *channel,
					      GList *recall_id_list)
{
  AgsAudio *audio;
  AgsChannel *current;
...
void
ags_channel_recursive_resolve_recall_down_input(AgsChannel *output,
						GList *recall_id_list)
{
  AgsAudio *audio;
  AgsChannel *current;
...
void
ags_channel_recursive_resolve_recall_down(AgsChannel *current,
					  GList *recall_id_list)
{
  AgsAudio *audio;
  GList *next_recall_id_list, *default_recall_id_list;
...
void
ags_channel_tillrecycling_resolve_recall_up(AgsChannel *channel,
					    GList *recall_id_list)
{
  AgsAudio *audio;
  AgsChannel *current;
...
void
ags_channel_recursive_init_recall_down_input(AgsChannel *output,
					     GList *recall_id_list,
					     gint stage)
{
  AgsAudio *audio;
...
void
ags_channel_recursive_init_recall_down(AgsChannel *current,
				       GList *recall_id_list,
				       gint stage)
{
  AgsAudio *audio;
...
void
ags_channel_tillrecycling_init_recall_up(AgsChannel *channel,
					 GList *recall_id_list,
					 gint stage)
{
  AgsAudio *audio;
